...
function Timesheet() {
  ...
  // 1. New state variables for date range filtering [cite: 42, 45]
  ...
  // 2. Updated fetch function to handle optional filters
  const fetchTimesheet = async (start = '', end = '') => {
    ...
    try {
      // Constructs the URL with query parameters for the backend
  ...
  // 3. Trigger for the filter button 
  const handleFilter = () => {
  ...
  return (
    <div className="timesheet-container">
      <div className="timesheet-card">
        ...
        {/* 4. Filter UI: Added to satisfy the "month or date range" requirement  */}
        <div className="filter-section" style={{ marginBottom: '20px', display: 'flex', gap: '10px', alignItems: 'center' }}>
          ...
          <button 
            onClick={() => { setStartDate(''); setEndDate(''); fetchTimesheet(); }} 
